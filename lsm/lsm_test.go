...
func baseTest(t *testing.T, lsm *LSM, n int) {
	// 用来跟踪调试的
	e := &utils.Entry{
		Key:       []byte("CRTS😁张欣MrGSBtL12345678"),
		Value:     []byte("hImkq95pkCRARFlUoQpCYUiNWYV9lkOd9xiUs0XtFNdOZe5siJVcxjc6j3E5LUng~=+%^*/()[]{}/!@#$?|©®😁😭🉑️🐂㎡张欣"),
		ExpiresAt: 0,
	}
...
func randStr(length int) string {
	// 包括特殊字符,进行测试
	str := "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ~=+%^*/()[]{}/!@#$?|©®😁😭🉑️🐂㎡张欣"
	bytes := []byte(str)
	result := []byte{}
