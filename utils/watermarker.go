...
// waterMark 保存了当前 xkv 里的所有活跃事务的 `ts`. 开始事务的时候需要添加 readTs 到 waterMark 里,
// 事务在执行后 `Done` 时添加 commitTs 到 waterMark 里. waterMark 在经过堆排序后, 取最小的 ts 为 `doneUtil` 值.
// 在进行事务清理时, 会以 waterMark 水位线的 doneUntil 为基准, 删除之前的已提交事务.
//
